        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                    ...
                    <el-form-item :label="contents.inputTitle == 1 ? '物品名称' : ''">
                        <el-input prefix-icon="el-icon-search" v-model="searchForm.xunwuqishiName" placeholder="物品名称" clearable></el-input>
                    </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        <el-button
                                v-if="isAuth('xunwuqishiLiuyan','新增')"
                                type="success"
                                icon="el-icon-plus"
                        ...
                        &nbsp;
                        <el-button
                                v-if="isAuth('xunwuqishiLiuyan','删除')"
                        ...
                        &nbsp;
                        <el-button
                                v-if="isAuth('xunwuqishiLiuyan','报表')"
                                type="success"
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('xunwuqishiLiuyan','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/xiaoyuanshiwu/upload/xunwuqishiLiuyanMuBan.xls"
                        >批量导入寻物启事留言数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('xunwuqishiLiuyan','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="xunwuqishiLiuyanUploadSuccess"
                                :on-error="xunwuqishiLiuyanUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('xunwuqishiLiuyan','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入寻物启事留言数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('xunwuqishiLiuyan','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "xunwuqishiLiuyan.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                          ...
                          v-if="isAuth('xunwuqishiLiuyan','查看')"
                          :data="dataList"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="xunwuqishiLiuyanText"
                                      header-align="center"
                                      label="留言内容">
                        <template slot-scope="scope">
                            <span v-if="scope.row.xunwuqishiLiuyanText != null &&scope.row.xunwuqishiLiuyanText.length>10">
                                {{scope.row.xunwuqishiLiuyanText.slice(0,10)}}...
                            </span>
                            <span v-else>
                                {{scope.row.xunwuqishiLiuyanText}}
                            </span>
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('xunwuqishiLiuyan','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('xunwuqishiLiuyan','修改') && sessionTable=='yonghu' && userId==scope.row.xunwuqishiYonghuId" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">回复</el-button>
                            <el-button v-if="isAuth('xunwuqishiLiuyan','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //级联表下拉框搜索条件
              ...
              zhaohuizhuangtaiTypesSelectSearch : [],
    ...
    //当前表下拉框搜索条件
            ...
            json_fields: {
                //级联表字段
                     ...
                     '找回状态': 'zhaohuizhuangtaiTypes',
                     '物品图片': 'xunwuqishiPhoto',
                     '丢失时间': 'xunwuqishiTime',
                     '丢失地址': 'xunwuqishiDizhi',
        ...
        methods: {
            chartDialog() {
                ...
                this.$nextTick(()=>{
                    ...
                    let params = {
                        ...
                        groupColumn: "zhaohuizhuangtai_types",
            ...
            getDataList() {
                ...
                if (this.searchForm.xunwuqishiName!= '' && this.searchForm.xunwuqishiName!= undefined) {
                ...
                if (this.searchForm.zhaohuizhuangtaiTypes!= '' && this.searchForm.zhaohuizhuangtaiTypes!= undefined) {
                    params['zhaohuizhuangtaiTypes'] = this.searchForm.zhaohuizhuangtaiTypes
                }
                ...
                params['xunwuqishiLiuyanDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "xunwuqishiLiuyan/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
                ...
                this.$http({
                    url: "dictionary/page?dicCode=zhaohuizhuangtai_types&page=1&limit=100",
                    method: "get",
                    page: 1,
                    limit: 100,
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.zhaohuizhuangtaiTypesSelectSearch = data.data.list;
                    }
                });
                this.$http({
            ...
            // 导入功能上传文件成功后调用导入方法
            xunwuqishiLiuyanUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "xunwuqishiLiuyan/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
                            message: "导入寻物启事留言数据成功",
                            type: "success",
                            duration: 1500,
                            onClose: () => {
                                _this.search();
            ...
            // 导入功能上传文件失败后调用导入方法
            xunwuqishiLiuyanUploadError(data){
            ...
            wuyong(id) {
                ...
                _this.$confirm(`确定    操作?`, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"
                    ...
                    _this.$http({
                        url: "xunwuqishiLiuyan/update",
                        method: "post",
                        data: {
                            id:id,
//                            xunwuqishiLiuyanTypes:xunwuqishiLiuyanTypes,
                        }
                    }).then(({data}) => {
                        if(data && data.code === 0){
